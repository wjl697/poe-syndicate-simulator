# 🎲 不朽辛迪加 (Immortals Syndicate) 解密推演游戏
...
> 一款基于经典黑帮势力推演机制制作的 **不朽辛迪加 (Syndicate) 独立解密与推演模拟游戏**。
> 本项目旨在复刻并还原丰富的同人逻辑推演体验，提供经典对抗、自由沙盒预设、画板连线推演及新手教学等丰富模式。

---

## 🖼️ 游戏界面预览 (Screenshots)

| ⚔️ 经典遭遇模式 | 🧪 自由沙盒模式 | 🎨 交互画板模式 |
|:---:|:---:|:---:|
| ![经典模式](经典模式.png) | ![沙盒模式](沙盒模式.png) | ![画板模式](画板模式.png) |

---

## ⚠️ 重要免责与版权声明 (Disclaimer & Fan Content Policy)

> [!IMPORTANT]
> 1. **独立同人创作**：本项目纯属**个人出于兴趣爱好**独立开发制作的粉丝同人作品，**与 Grinding Gear Games (GGG) 及其母公司腾讯无任何官方关联、赞助或认可关系**。
> 2. **美术与视觉资产归属**：游戏中所使用的相关素材、图标、概念设计与商标等视觉资产，**其原作者版权与知识产权均归 Grinding Gear Games (GGG) 所有**。
> 3. **完全非商业化**：本项目为**100% 完全免费且开源**的同人项目，**绝不包含任何形式的商业化盈利、道具收费、广告变现或付费买断行为**。
...
### 4. 📚 游戏内置百科大全与教程系统
- **嵌入式 Markdown 阅读引擎**：内置完整的新手入门指南，章节目录跳转精准支持 **Top Position（视口最上方顶端对齐）**；
- **内嵌文本零丢失双保险**：兼顾本地开发与打包 `.exe` 独立运行，文件读取失败时自动切换内置文本；
- **作者联系与赞赏弹窗**：支持跳转 B 站主页，多格式兼容识别展示赞赏码图片 `赞赏码.jpg`。
...
2. **强制衍生代码开源（强传染性）**：任何使用、衍生或修改本项目代码而生成的新软件/游戏，**也必须以 GPL-3.0 协议全量开源其源代码**，严禁将其拿去进行二次闭源包装或打包卖钱。
...
- **B站主页**：[@Ka3m](https://space.bilibili.com/99815426)
