...
@export_group("Text formats")
## Custom color for bold text (leave WHITE for default).
@export var bold_color: Color = Color.WHITE :
	set(new_value):
		bold_color = new_value
		queue_update()

@export_group("Table formats")

## Border color of Markdown tables.
@export var table_border_color: Color = Color(0.7, 0.58, 0.28, 0.8) :
	set(new_value):
		table_border_color = new_value
		queue_update()
## Background color of Markdown tables.
@export var table_background_color: Color = Color(0.15, 0.15, 0.2, 0.6) :
	set(new_value):
		table_background_color = new_value
		queue_update()
...
func _on_meta_clicked(meta: Variant) -> void:
	...
	# 内部锚点处理：以 '#' 开头，必定为文档内部章节定位
	if meta.begins_with("#"):
		scroll_to_anchor(meta as String)
		return
...
## 根据锚点名或章节标题名滚动定位到对应章节段落并精准置顶
func scroll_to_anchor(target: String) -> bool:
	var clean_target := target.lstrip("#").strip_edges().to_lower()
	var target_p := -1
	
	# 1. 精确匹配
	if target in _header_anchor_paragraph:
		target_p = _header_anchor_paragraph[target]
	else:
		var hash_target := "#" + clean_target
		if hash_target in _header_anchor_paragraph:
			target_p = _header_anchor_paragraph[hash_target]
		else:
			# 2. 模糊/包含匹配 (忽略符号、括号与空格)
			var simplify_str := func(s: String) -> String:
				return s.replace("、","").replace(" ","").replace("（","").replace("）","").replace("(","").replace(")","").replace("-","")
			
			var search_simple := simplify_str.call(clean_target)
			for anchor_key in _header_anchor_paragraph:
				var key_simple := simplify_str.call((anchor_key as String).lstrip("#").to_lower())
				if search_simple in key_simple or key_simple in search_simple:
					target_p = _header_anchor_paragraph[anchor_key]
					break

	if target_p >= 0:
		var vsb = get_v_scroll_bar()
		if vsb:
			# 先将滚动位置定位在目标下方（底部），强制 RichTextLabel 向上滚动对齐顶端
			vsb.value = vsb.max_value
		self.scroll_to_paragraph(target_p)
		return true
...
func _process_text_formatting_syntax(line: String) -> String:
	...
	while true:
		...
		if not result:
			break
		var match_content := result.get_string(2)
		var b_open := "[b]"
		var b_close := "[/b]"
		if bold_color != Color.WHITE:
			b_open = "[color=#%s]" % bold_color.to_html()
			b_close = "[/color]"
		var replacement := b_open + match_content + b_close
		processed_line = processed_line.substr(0, result.get_start()) + replacement + processed_line.substr(result.get_end())

		_debug("... bold text: " + match_content)
...
func _process_table_syntax(line: String) -> String:
	...
	# 判断是否具备表格行的基本特征
	var is_table_row := false
	if stripped_line.contains("|"):
		if stripped_line.begins_with("|") or stripped_line.ends_with("|"):
			is_table_row = true
		else:
			# 形如 "A | B" 的无首尾竖线格式
			is_table_row = stripped_line.count("|") >= 1
	...
	for cell in raw_cells:
		var c_str := cell.strip_edges()
		if c_str.is_empty():
			continue
		for i in range(c_str.length()):
			var ch := c_str[i]
			if ch != "-" and ch != ":" and ch != " ":
				is_delimiter = false
				break
	...
	if not _within_table:
		# Godot 4 富文本 BBCode 的 table 格式为 [table=列数]
		processed_line += "[table=%d]" % raw_cells.size()
		_within_table = true

	var border_attr := ""
	if table_border_color.a > 0.01:
		border_attr = " border=#" + table_border_color.to_html()

	var bg_attr := ""
	var header_bg_attr := ""
	if table_background_color.a > 0.01:
		bg_attr = " bg=#" + table_background_color.to_html()
		var h_bg := Color(
			clampf(table_background_color.r * 1.5 + 0.06, 0.0, 1.0),
			clampf(table_background_color.g * 1.5 + 0.06, 0.0, 1.0),
			clampf(table_background_color.b * 1.5 + 0.1, 0.0, 1.0),
			minf(table_background_color.a + 0.25, 0.95)
		)
		header_bg_attr = " bg=#" + h_bg.to_html()
	else:
		header_bg_attr = " bg=#" + Color(0.15, 0.15, 0.22, 0.5).to_html()

	for cell in raw_cells:
		var content := cell.strip_edges()
		if _table_row == 0:
			processed_line += "[cell%s%s][center] %s [/center][/cell]" % [border_attr, header_bg_attr, content]
		else:
			processed_line += "[cell%s%s] %s [/cell]" % [border_attr, bg_attr, content]
