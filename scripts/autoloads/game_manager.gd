...
func raid_safehouse(div: int):
	...
	# ===== 步骤 2：原首领剥夺职务与降级 =====
	var old_leader = get_division_leader(div)
	if old_leader:
		old_leader.is_leader = false
		old_leader.division = Division.NONE
		old_leader.rank = 0
		print("[安全屋通关] 原首领降级为0星自由人: ", old_leader.member_name)

	# ===== 步骤 3：提拔临时首领 =====
	var active_subs: Array = []
	for mname in members:
		var m: MemberState = members[mname]
		if m.division == div and not m.is_leader and m.is_on_board and not m.is_imprisoned:
			active_subs.append(m)

	var temp_leader: MemberState = null
	if not active_subs.is_empty():
		...
		temp_leader = best_subs[randi() % best_subs.size()]
		temp_leader.is_leader = true
		print("[安全屋通关] 提拔临时首领: ", temp_leader.member_name, " (星级=", temp_leader.rank, ")")

	# ===== 步骤 4：同星级跨部门下属位置互换 =====
	var remaining_subs: Array = []
	for sub in active_subs:
		if temp_leader != null and sub.member_name == temp_leader.member_name:
			continue
		remaining_subs.append(sub)

	if not remaining_subs.is_empty():
		var max_rem_rank := -1
		for sub in remaining_subs:
			if sub.rank > max_rem_rank:
				max_rem_rank = sub.rank
		var candidates_to_swap: Array = []
		for sub in remaining_subs:
			if sub.rank == max_rem_rank:
				candidates_to_swap.append(sub)
		var local_swap_sub: MemberState = candidates_to_swap[randi() % candidates_to_swap.size()]

		# 在其他部门找同星级的“普通下属（非首领）”
		var other_div_same_rank_subs: Array = []
		for mname in members:
			var m: MemberState = members[mname]
			if m.division != Division.NONE and m.division != div and not m.is_leader and m.is_on_board and not m.is_imprisoned:
				if m.rank == local_swap_sub.rank:
					other_div_same_rank_subs.append(m)

		if not other_div_same_rank_subs.is_empty():
			var target_swap_sub: MemberState = other_div_same_rank_subs[randi() % other_div_same_rank_subs.size()]
			var orig_div_local = local_swap_sub.division
			var orig_div_target = target_swap_sub.division
			local_swap_sub.division = orig_div_target
			target_swap_sub.division = orig_div_local
			print("[安全屋通关] 部门下属互换: ", local_swap_sub.member_name, " 去往部门 ", orig_div_target, " | ", target_swap_sub.member_name, " 来到部门 ", orig_div_local)

	# ===== 步骤 5：自由人空降为正式首领，临时首领退回为下属 =====
	var free_agents: Array = get_unassigned_members()
	var valid_free: Array = []
	for fa in free_agents:
		if not fa.is_imprisoned:
			valid_free.append(fa)

	var new_official_leader: MemberState = null
	if not valid_free.is_empty():
		new_official_leader = valid_free[randi() % valid_free.size()]
		new_official_leader.division = div
		new_official_leader.is_leader = true
		new_official_leader.rank = 1
		print("[安全屋通关] 自由人空降为正式首领: ", new_official_leader.member_name, " (重置为 1 星)")

	if temp_leader != null:
		temp_leader.is_leader = false
		print("[安全屋通关] 临时首领退回为普通下属: ", temp_leader.member_name, " (保持 ", temp_leader.rank, " 星)")

	# ===== 步骤 6：清场——除去首领和退下来的临时首领，其余成员全部变为自由人 =====
	var members_in_div: Array = []
	for mname in members:
		var m: MemberState = members[mname]
		if m.division == div and m.is_on_board and not m.is_imprisoned:
			members_in_div.append(m)


	for m in members_in_div:
		var is_official_leader = (new_official_leader != null and m.member_name == new_official_leader.member_name)
		var is_returned_temp_leader = (temp_leader != null and m.member_name == temp_leader.member_name)
		if not is_official_leader and not is_returned_temp_leader:
			m.division = Division.NONE
			m.is_leader = false
			m.rank = 0
			print("[安全屋通关] 清场: 成员 ", m.member_name, " 变为0星自由人")
