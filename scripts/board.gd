...
func _on_board_changed():
	...
	# 4. 如果处于沙盒向导步骤一，则显式隐藏主盘面卡牌与连线
	var wizards = get_tree().get_nodes_in_group("sandbox_wizard")
	if wizards.size() > 0:
		var wizard = wizards[0]
		if is_instance_valid(wizard) and wizard._current_step == 1:
			set_sandbox_cards_visible(false)

	# 5. 重新解析坑位并全量布局（精准将卡牌排布到点击的对应区域坑位）
	_parse_custom_slots()
	_layout_cards()

func set_sandbox_cards_visible(is_vis: bool):
	for mname in _cards:
		var card = _cards[mname]
		if is_instance_valid(card):
			card.visible = is_vis
	if is_instance_valid(_rel_lines):
		_rel_lines.visible = is_vis
