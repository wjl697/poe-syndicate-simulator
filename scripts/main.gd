		...
		if m:
			match slot_type:
				...
				"PRISON":
					...
					m.is_imprisoned = true
					m.is_leader = false
					m.rank = maxi(m.rank, 1) # 保证在押人员至少拥有 1 星，且支持自由设置星级
					m.prison_turns_left = 3
...
func _place_whiteboard_card_at_slot(slot_type: String, target_div: int):
	...
	match slot_type:
		...
		"SUBORDINATE":
			...
			candidate.is_imprisoned = false
		"PRISON":
			candidate.division = GameManager.Division.NONE # 默认没有归属部门
			candidate.is_leader = false
			candidate.rank = 1 # 新放置于审讯位：默认 1 星
			candidate.is_imprisoned = true # 正确标记为在押人员
