...
func update_display() -> void:
	...
	# 星级 — 815x447 图层，直接对齐背景 (仅非在押且部门为 NONE 的纯自由人强制无星级)
	var is_pure_free = (member_data.division == GameManager.Division.NONE and not member_data.is_imprisoned)
	if (not is_abstract_mode and is_hidden) or member_data.rank <= 0 or is_pure_free:
