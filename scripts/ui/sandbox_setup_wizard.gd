...
func _update_step_ui():
	...
	if board != null and is_instance_valid(board):
		...
		if board.has_method("set_sandbox_cards_visible"):
			board.set_sandbox_cards_visible(_current_step != 1)
		GameManager.board_changed.emit()
...
func _show_presets_management_popup():
	...
	delete_btn.pressed.connect(func():
		if _presets_dict["presets"].has(_active_preset_name):
			var deleted_name: String = _active_preset_name
			_presets_dict["presets"].erase(deleted_name)
			...
			if keys.is_empty():
				...
				_active_preset_name = keys[0]
				_presets_dict["active_preset"] = _active_preset_name
				# 修正 BUG：仅更新活动预设名称，绝对不重新加载破坏玩家当前的盘面布置！
...
func _load_preset(preset_name: String):
	...
	if benched_preset is Array and benched_preset.size() == 3:
		for mname in benched_preset:
			_benched_members.append(mname)
	# 如果预设中没有显式保存 3 名替补名单，则保持替补槽为空，交由玩家在步骤 1 中手动挑选，决不随机乱塞！
